...
{
    if (debug_set.empty() && !debug_params.empty())
        delimiter_set(debug_params, debug_set);
}
...
int main(int argc, char *const *argv)
{
    ...
    if (!pipe_in)
    {
        ...
        if (get_debug_flag("sighash", debug_set))
            btc_sighash_logf = btc_logf_stderr;
        if (get_debug_flag("signing", debug_set, true))
            btc_sign_logf = btc_logf_stderr;
        if (get_debug_flag("segwit", debug_set, true))
            btc_segwit_logf = btc_logf_stderr;
        if (get_debug_flag("taproot", debug_set, true))
            btc_taproot_logf = btc_logf_stderr;
        btc_logf("LOG:");
        if (btc_enabled(btc_sighash_logf))
            btc_logf(" sighash");
        if (btc_enabled(btc_sign_logf))
            btc_logf(" signing");
        if (btc_enabled(btc_segwit_logf))
            btc_logf(" segwit");
        if (btc_enabled(btc_taproot_logf))
            btc_logf(" taproot");
        btc_logf("\n");
    ...
    if (ca.m.count('f'))
    {
        ...
        if (verbose)
            fprintf(stderr, "resulting flags:\n・ %s\n", svf_string(flags, "\n・ ").c_str());
    }
    ...
    {
        p2sh_script_payload = vchPushValue;
        ++count;
    }
    ...
    if (instance.successor_script.size())
    {
        ...
        while (instance.successor_script.GetOp(it, opcode, vchPushValue))
            ++count;
    ...
    if (has_p2sh)
    {
        ...
        while (p2sh_script.GetOp(it, opcode, vchPushValue))
            ++count;
    }
    ...
    for (size_t siter = 0; siter < script_ptrs.size(); ++siter)
    {
        ...
        if (header != "")
            script_lines[i++] = strdup(header.c_str());
        it = script->begin();
    ...
    if (instance.has_preamble)
    {
        if (verbose)
            ...
            btc_logf("note: there is a for-clarity preamble (use --verbose for details)\n");
    }
