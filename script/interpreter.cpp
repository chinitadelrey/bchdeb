...
// popstack is in debugger/interpreter.h
...
static bool EvalChecksig(ScriptExecutionEnvironment &env, const valtype &sig, const valtype &pubkey, CScript::const_iterator pbegincodehash, CScript::const_iterator pend, ScriptExecutionData &execdata, unsigned int flags, const BaseSignatureChecker &checker, SigVersion sigversion, ScriptError *serror, bool &success)
{
    ...
    switch (sigversion)
    {
        // TODO: need to get rid of this segwit stuff, or at least, make sense of it.
    ...
    {
        {
            ...
            if (!env.allow_disabled_opcodes && (opcode == OP_CAT ||
                                                opcode == OP_SUBSTR ||
                                                opcode == OP_LEFT ||
                                                opcode == OP_RIGHT ||
                                                opcode == OP_INVERT ||
            ...
            if (fExec && 0 <= opcode && opcode <= OP_PUSHDATA4)
            {
                ...
                break;
                    ...
                    //     bool success = true;
                    //     if (!EvalChecksig(env, sig, pubkey, pbegincodehash, pend, execdata, flags, checker, sigversion, serror, success))
                    //         return false;
                    //     popstack(stack);
                    //     popstack(stack);
                    //     popstack(stack);
                ...
                case OP_CHECKMULTISIGVERIFY:
                {
                    ...
                    while (fSuccess && nSigsCount > 0)
                    {
                        ...
                        if (pretend_valid_pubkeys.count(vchPubKey))
                        {
                            ...
                            if (!fOk)
                                btc_sign_logf("- [mock] wrong pubkey for sig; marking as failed\n");
                        }
                        else
                        {
                    ...
                    if ((flags & SCRIPT_VERIFY_NULLDUMMY) && stacktop(-1).size())
                    {
                        if (btcdeb_verbose)
                            ...
                            printf("Hint: with Segwit activation, the OP_CHECKMULTISIG extra argument must be empty (--verbose for details)\n");
...
ScriptExecutionEnvironment::ScriptExecutionEnvironment(std::vector<std::vector<unsigned char>> &stack_in, const CScript &script_in, unsigned int flags_in, const BaseSignatureChecker &checker_in)
    : script(script_in), pend(script.end()), pbegincodehash(script.begin()), nOpCount(0), fRequireMinimal((flags_in & SCRIPT_VERIFY_MINIMALDATA) != 0), stack(stack_in), flags(flags_in), checker(checker_in), opcode_pos(0), execdata{}, allow_disabled_opcodes{false}
{
 ...
 */
    ...
    public:
        ...
        void SerializeScriptCode(S &s) const
        {
            ...
            btc_sign_logf(" << script:");
            print_vec(scriptCode, btc_sign_logf);
            btc_sign_logf("\n");
            it = itBegin;
            while (scriptCode.GetOp(it, opcode))
            {
...
void PrecomputedTransactionData::Init(const T &txTo, std::vector<CTxOut> &&spent_outputs, bool force)
{
    ...
    for (size_t inpos = 0; inpos < txTo.vin.size() && !(uses_bip143_segwit && uses_bip341_taproot); ++inpos)
    {
        ...
        if (uses_bip341_taproot && uses_bip143_segwit)
            break; // No need to scan further if we already need all.
    }
...
bool SignatureHashSchnorr(uint256 &hash_out, const ScriptExecutionData &execdata, const T &tx_to, uint32_t in_pos, uint8_t hash_type, SigVersion sigversion, const PrecomputedTransactionData &cache, MissingDataBehavior mdb)
{
    ...
    if (!(cache.m_bip341_taproot_ready && cache.m_spent_outputs_ready))
    {
        ...
        {
            btc_sign_logf("- BIP341-taproot not ready\n");
        }
...
bool GenericTransactionSignatureChecker<T>::CheckECDSASignature(const std::vector<unsigned char> &vchSigIn, const std::vector<unsigned char> &vchPubKey, const CScript &scriptCode, SigVersion sigversion) const
{
    ...
    btc_sign_logf("  sig         = ");
    print_vec(vchSigIn, btc_sign_logf);
    btc_sign_logf("\n");
    btc_sign_logf("  pub key     = ");
    print_vec(vchPubKey, btc_sign_logf);
    btc_sign_logf("\n");
    btc_sign_logf("  script code = ");
    print_vec(scriptCode, btc_sign_logf);
    btc_sign_logf("\n");
    CPubKey pubkey(vchPubKey);
    if (!pubkey.IsValid())
    ...
    {
        btc_sign_logf("- failed: witness sighashes need the amount");
...
uint256 ComputeTaprootMerkleRoot(Span<const unsigned char> control, const uint256 &tapleaf_hash)
{
    ...
    std::string k_desc = strprintf("TapLeaf(0x%02x || ?)", uint8_t(control[0] & TAPROOT_LEAF_MASK)); // , HexStr(script).c_str()
    btc_taproot_logf("  (%s)\n", k_desc.c_str());
...
static bool VerifyTaprootCommitment(const std::vector<unsigned char> &control, const std::vector<unsigned char> &program, const uint256 &tapleaf_hash)
{
    ...
    btc_taproot_logf("- merkle root  = %s\n", merkle_root.ToString().c_str());
    // Verify that the output pubkey matches the tweaked internal pubkey, after correcting for parity.
    bool res = q.CheckTapTweak(p, merkle_root, control[0] & 1);
    btc_taproot_logf("- q.CheckTapTweak(p, merkle_root, %d) == %s\n", control[0] & 1, res ? "success" : "failure");
    return res;
}
